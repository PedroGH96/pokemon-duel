...
public class BattleService {
    ...
    private double computeWheelAngle(PokemonFigure figure, Move drawnMove) {
        ...
        for (Move m : figure.getPokemon().getMoves()) {
            double segmentDegrees = (m.getPercentage() / 100.0) * 360.0;
            if (m == drawnMove) {
                targetCenter = angle + segmentDegrees / 2.0;
                break;
            }
            angle += segmentDegrees;
        }

        // Adiciona rotações extras para que a animação seja visível (720°)
        return 720.0 + targetCenter;
    }
